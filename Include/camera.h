...
// 动态路径解析：无视项目名称变化，通过3层策略自动查找资源文件
//   策略1: ./Data/<file>           (相对于 CWD)
//   策略2: <exe_dir>/Data/<file>    (通过 /proc/self/exe 获取可执行文件目录)
//   策略3: /tmp/Data/<file>         (通用回退，无项目名依赖)
extern void resolve_asset_path(char *out, size_t out_size, const char *relative_path);

// 录像存储路径 (使用 /tmp tmpfs 以适配嵌入式环境)
...
extern void mainloop(void);// [已弃用] 主循环，请使用 backup_camera_thread
extern void *backup_camera_thread(void *arg);  // 后视摄像头线程(非阻塞)
